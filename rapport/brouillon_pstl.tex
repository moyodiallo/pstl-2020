...
Deux ordres de parcours peuvent être considérés, \(post_{ordre}\) et \(pre_{ordre}\).
...
\textbf{Profil d'un ensemble}: Le profile P=profile(S) d'un ensemble S est simplement une liste du nombre d'élément de S ayant un certain indexe, on note P = (p0,p1…,pj,…pk) où pj est le nombre d'elements dans l'ensemble S d'index j et k l'index maximal de S. 
...
En s'appuyant sur cette methode on peut definir une decomposition sur un arbre de taille fixé n, cette methode s'apuie sur le fait que \(b_n\) est definie en une somme de produit, où chaque element de ce produit peut être vu comme la representation d'un arbre dont ayant un sous arbre gauche et un sous arbre droit de taille fixe. Par exemple \(B_7\) = 429, et on peut le decomposer en:
...
Ainsi, si je cherche l'arbre n°250, j'obtiens : 250 = 132 + 42 + 28 + 25 + 23. On trouve donc que l'arbre 250 est composé
...
En s'inspirant de cette methode on definie une methode DECOMPOSE(r\textquoteright,n\textquoteright,s,p\textquoteright) permettant de generer un bdd de taille n\textquoteright, ayant un profil p\textquoteright et un rang r\textquoteright. s est une variable contenant le spine du bdd a generer.
  ...
  \begin{algorithmic}[1]
    \Statex
        \Function{decompose}{$rank, n, spine, p$}
            ...
            \State \Let{$s \gets spine.$get\_profile$()$}
            ...
            \While{$i \geq 0$}
                ...
                \For{$(l,w0)$ in $d0.$items$()$}
                    \State \Let{$q\_prim \gets $sum\_profile$(q,l) $}
                    \If{$n-1-i = 0$}
                    ...
                    \For{$(h,w1) in d1.items()$}
                        \State \Let{$t \gets sum\_profile(sum\_profile(e\_k(k),list(l)),list(h))$}
...
On decrit ensuite un algorithme GENERATE(r,n,s,p) permettant de generer un spine de taille n, de profil p et de rang r. La variable s est une variable globale qui contiendra en fin d'execution le bdd generé.
Cet algorithme prend en argument un spine s contenant les noeuds initialement dans la pool (ici \(\up\) et \(\bot\)) et crée au fur et a mesure le bdd souhaité en faisant appel a la fonction decompose et en mettant a jour le spine.
...
Dans la fonction generate on fait en sorte d'identifier les noeuds que l'on peut assigner aux nil-transitions du noeud courant. Pour un noeud v ayant deux nil-trnasitions on verifie donc avant de lui assigner en tant que haut fils un noeud n et fils bas un noeud m que I(m)<I(v) et I(n)<I(v), on verifie egalement qu'il n'existe pas v\textquoteright \(\in\) spine tel que v!=v\textquoteright, où v\textquoteright a pour fils bas m\textquoteright et pour fils haut n\textquoteright, que m=m\textquoteright et n=n\textquoteright.

On fait ensuite de même lorsque le noeud courant a une seul nil-transitions en tant que fils bas ou en tant que fils haut tout en respectant les regle que l'on a deja decrit pour la generation des bdd. 
...
%    \item get\_profile() : Cette fonction retourne retourne le profile du spine courant.
...
Enfin on decrit un algorithme GEN\_BDD(r,n,k) permettant de generer un bdd de taille n, d'index k et de rang r en utilisant la fonction generate decrite precedemment.
...
Spine est ici une classe d'objet permettant de representer un spine.
